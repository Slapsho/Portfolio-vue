<template>
  <div class="work-view">
    ...
    <transition name="fade">
      <div v-if="showModal" class="modal" @click.self="closeModal" aria-hidden="true" @keydown.esc="closeModal">
        <div class="modal-content" @click.stop>
          <button class="close-modal" @click="closeModal" aria-label="Fermer la modale">&times;</button>
          <img :src="selectedImage.src" :alt="selectedImage.alt" />
          <p>{{ selectedImage.description }}</p>
          <!-- Ajout de boutons supplémentaires pour plus d'informations -->
          <button @click="openAdditionalInfo('Détails Cahier des Charges', 'Explication sur ce travail spécifique')">Voir plus de détails</button>
        </div>
      </div>
    </transition>
